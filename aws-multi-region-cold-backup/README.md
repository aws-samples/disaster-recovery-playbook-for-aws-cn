...
按照上述方案，在[GitHub aws-dr-samples repo](https://github.com/aws-samples/disaster-recovery-playbook-for-aws-cn)上提供了基于[Terraform](https://www.terraform.io/)的可执行脚本，该套脚本可以帮助用户快速构建灾备环境。用户根据所需创建的资源写成脚本文件。执行脚本时，Terraform 通过调用 AWS API 来快速构建 AWS 资源。下文有如何使用的详细步骤描述。
